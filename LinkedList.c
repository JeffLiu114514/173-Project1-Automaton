 ...
 */
void LinkedList_add_at_front(LinkedList this, int data) {
 ...
 */
void LinkedList_add_at_end(LinkedList this, int data) {
	Node node = new_Node(data);
	node->prev = this->last;
	if (this->last != NULL) {
		this->last->next = node;
 ...
 */
bool LinkedList_contains(const LinkedList this, int data) {
...
bool LinkedList_containsAll(LinkedList list1, LinkedList list2) {
    for (Node node2 = list2->first; node2 != NULL; node2=node2->next) {
        if (!LinkedList_contains(list1, node2->data)) {
            return false;
        }
    }
    return true;
...
bool LinkedList_equals(LinkedList list1, LinkedList list2) {
    return LinkedList_containsAll(list1, list2) && LinkedList_containsAll(list2, list1);
}
 ...
 */
void LinkedList_remove(LinkedList this, int data) {
	for (Node node=this->first; node != NULL; node=node->next) {
		if (node->data == data) {
			if (node == this->first) {
 ...
 */
int LinkedList_elementAt(LinkedList this, int index) {
	int i = 0;
	for (Node node=this->first; node != NULL; node=node->next) {
		if (i == index) {
			return node->data;
 ...
 */
int LinkedList_pop(LinkedList this) {
	int data = LinkedList_elementAt(this, 0);
 ...
 */
void LinkedList_iterate(const LinkedList this, void (*func)(int)) {
	for (Node node=this->first; node != NULL; node=node->next) {
		func(node->data);
	}
}
 ...
 */
int LinkedListIterator_next(LinkedListIterator this) {
